...
public class PermissionManager {
	 ...
	 * 检查位置权限
	 */
	public List<String> checkPermissionLocation() {
		if (ContextUtil.isAndroidM()) {
	 ...
	 * 检查存储权限
	 */
	public List<String> checkPermissionStorage() {
		if (ContextUtil.isAndroidM()) {
			List<String> list = new ArrayList<String>(2);
			//存储空间
			boolean readStorage =
	 ...
	 * 检查电话权限
	 */
	public List<String> checkPermissionPhone() {
		if (ContextUtil.isAndroidM()) {
			List<String> list = new ArrayList<String>(1);
			//电话
			boolean callPhone = context.checkSelfPermission(Manifest.permission.CALL_PHONE) == PackageManager.PERMISSION_GRANTED;
			if (!callPhone) {
	 ...
	 * 检查相机权限
	 */
	public List<String> checkPermissionCamera() {
		if (ContextUtil.isAndroidM()) {
			List<String> list = new ArrayList<String>(1);
			//相机
			boolean camera = context.checkSelfPermission(Manifest.permission.CAMERA) == PackageManager.PERMISSION_GRANTED;
			if (!camera) {
	...
	public List<String> checkPermissionMicroPhone() {
		if (ContextUtil.isAndroidM()) {
			List<String> list = new ArrayList<String>(1);
			//麦克风
			boolean microPhone = context.checkSelfPermission(Manifest.permission.RECORD_AUDIO) == PackageManager.PERMISSION_GRANTED;
			if (!microPhone) {
	...
	public void requestPermission(Activity activity, String permissionGroup) {
		if (null != activity && StringUtil.isNotEmpty(permissionGroup)) {
			if (Manifest.permission_group.LOCATION.equals(permissionGroup)) {
				List<String> list = MyApplication.getInstance().getPermissionManager().checkPermissionLocation();
				if (CollectionUtil.isNotEmpty(list)) {
					activity.requestPermissions(CollectionUtil.convertListToArray(list),
												ReqCodeKey.REQUEST_CODE_PERMISSION_LOCATION);
				}
			} else if (Manifest.permission_group.STORAGE.equals(permissionGroup)) {
				List<String> list = MyApplication.getInstance().getPermissionManager().checkPermissionStorage();
				if (CollectionUtil.isNotEmpty(list)) {
					activity.requestPermissions(CollectionUtil.convertListToArray(list),
												ReqCodeKey.REQUEST_CODE_PERMISSION_STORAGE);
				}

			} else if (Manifest.permission_group.PHONE.equals(permissionGroup)) {
				List<String> list = MyApplication.getInstance().getPermissionManager().checkPermissionPhone();
				if (CollectionUtil.isNotEmpty(list)) {
					activity.requestPermissions(CollectionUtil.convertListToArray(list), ReqCodeKey.REQUEST_CODE_PERMISSION_PHONE);
				}

			} else if (Manifest.permission_group.CAMERA.equals(permissionGroup)) {
				List<String> list = MyApplication.getInstance().getPermissionManager().checkPermissionCamera();
				if (CollectionUtil.isNotEmpty(list)) {
					activity.requestPermissions(CollectionUtil.convertListToArray(list), ReqCodeKey.REQUEST_CODE_PERMISSION_CAMERA);
				}

			} else if (Manifest.permission_group.MICROPHONE.equals(permissionGroup)) {
				List<String> list = MyApplication.getInstance().getPermissionManager().checkPermissionMicroPhone();
				if (CollectionUtil.isNotEmpty(list)) {
					activity.requestPermissions(CollectionUtil.convertListToArray(list),
												ReqCodeKey.REQUEST_CODE_PERMISSION_MICROPHONE);
				}
	...
	public static void removeAuthorizedPermission(List<String> list, int reqCode) {
		switch (reqCode) {
			case ReqCodeKey.REQUEST_CODE_PERMISSION_LOCATION:
				list.remove(Manifest.permission_group.LOCATION);
				break;
			case ReqCodeKey.REQUEST_CODE_PERMISSION_STORAGE:
				list.remove(Manifest.permission_group.STORAGE);
				break;
			case ReqCodeKey.REQUEST_CODE_PERMISSION_PHONE:
				list.remove(Manifest.permission_group.PHONE);
				break;
			case ReqCodeKey.REQUEST_CODE_PERMISSION_CAMERA:
				list.remove(Manifest.permission_group.CAMERA);
				break;
			case ReqCodeKey.REQUEST_CODE_PERMISSION_MICROPHONE:
