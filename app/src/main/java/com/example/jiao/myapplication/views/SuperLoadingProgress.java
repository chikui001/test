...
public class SuperLoadingProgress extends View {
	...
	private SuperLoadingProgressListener superLoadingProgressListener;

	public void addListener(SuperLoadingProgressListener listener) {
		this.superLoadingProgressListener = listener;
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		setMeasuredDimension(widthSpecSize + 10 * (int) strokeWidth, heightSpecSize + 10 * (int) strokeWidth);
	}
	...
	private void init(Context mContext, AttributeSet attrSet) {
		TypedArray array = mContext.obtainStyledAttributes(attrSet, R.styleable.SuperLoadingProgress);
		strokeWidth = array.getDimension(R.styleable.SuperLoadingProgress_draw_width, 20);
		...
		mTickAnimation.addListener(new AnimatorListenerAdapter() {
			...
			public void onAnimationEnd(Animator animation) {
				super.onAnimationEnd(animation);
				if (null != superLoadingProgressListener) {
					superLoadingProgressListener.mTickAnimationFinish();
				}
			}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (progress < maxProgress) {
			handler.removeMessages(ADD_PROGRESS);
			handler.sendEmptyMessageDelayed(ADD_PROGRESS, 10);
		} else if (!isSuccess) {
			finishSuccess();
		}
	}
	...
	protected void onDetachedFromWindow() {
		...
		if (null != mDownAnimation) {
			mDownAnimation.cancel();
		}
		if (null != mForkAnimation) {
			mForkAnimation.cancel();
		}
		if (null != mTickAnimation) {
			mTickAnimation.cancel();
		}
		if (null != mshockAnimation) {
			mshockAnimation.cancel();
		}
		if (null != mCommaAnimation) {
			mCommaAnimation.cancel();
		}
	}
	...
	private void drawSmallRectFly(Canvas canvas) {
		...
		float x2 = (float) (bigRadius * Math
				.cos(curSweepAngle + 0.05 * endAngle + 0.1 * endAngle * (1 - curSweepAngle / 0.9 * endAngle)));//
		float y2 = -(float) (bigRadius * Math
				.sin(curSweepAngle + 0.05 * endAngle + 0.1 * endAngle * (1 - curSweepAngle / 0.9 * endAngle)));
	...
	private void drawRectDown(Canvas canvas) {
		...
		//非交集
		Region region1 = new Region(Math.round(pos1[0]) - (int) strokeWidth / 4, Math.round(pos1[1]),
									Math.round(pos2[0] + (int) strokeWidth / 4), Math.round(pos2[1]));
		region1.op(mRect, Region.Op.DIFFERENCE);
		drawRegion(canvas, region1, downRectPaint);

		//交集
		Region region2 = new Region(Math.round(pos1[0]) - (int) strokeWidth / 2, Math.round(pos1[1]),
									Math.round(pos2[0] + (int) strokeWidth / 2), Math.round(pos2[1]));
	...
	private void drawFork(Canvas canvas) {
		...
		canvas.drawLine(2 * radius + strokeWidth, radius + strokeWidth, 2 * radius + strokeWidth, 2 * radius + strokeWidth,
						downRectPaint);
	...
	public void recoveryAndRestart() {
		this.progress = 0;
		this.status = 0;
		isSuccess = false;
		invalidate();
	...
	private static final int ADD_PROGRESS = 101;
	private MyHandler handler = new MyHandler(this);

	private static class MyHandler extends Handler {

		private WeakReference<SuperLoadingProgress> weakReference;

		public MyHandler(SuperLoadingProgress superLoadingProgress) {
			this.weakReference = new WeakReference<SuperLoadingProgress>(superLoadingProgress);
		}

		@Override
		public void handleMessage(Message msg) {
			super.handleMessage(msg);
			SuperLoadingProgress superLoadingProgress = weakReference.get();
			switch (msg.what) {
				case ADD_PROGRESS:
					if (null != superLoadingProgress) {
						superLoadingProgress.increaseProgress();
					}
					break;
			}
		}
